...
3. **After presenting questions (Step 2, stateless environment path)**: immediately save the questions to `./pbis/pbi-[feature-slug]/pbi-answers.md` as a placeholder (with empty answer fields). Instruct the user: *"Edite o arquivo `pbi-answers.md` com suas respostas e invoque `/do-create-pbi` novamente para continuar."*
...
1. Read the template at the path resolved in Step 0 (e.g., `.claude/skills/do-create-pbi/assets/pbi-template.md` for Claude Code, `.cursor/rules/do-create-pbi/assets/pbi-template.md` for Cursor AI).
...
1. **SYNC INTERNAL PROGRESS**: Once the PBI is saved, if `TaskUpdate` is available (Claude Code only; skip in Copilot and Cursor), use it to mark all corresponding items in your internal task tracking as `completed`. Otherwise, skip this step.
...
- If the template file is missing at the skills directory path (e.g., `.claude/skills/do-create-pbi/assets/pbi-template.md` for Claude Code, `.cursor/rules/do-create-pbi/assets/pbi-template.md` for Cursor AI), report the error and halt — do not generate a PBI without the template.
...
## References
- Template: resolved by environment (e.g., `.claude/skills/do-create-pbi/assets/pbi-template.md` for Claude Code, `.cursor/rules/do-create-pbi/assets/pbi-template.md` for Cursor AI)
