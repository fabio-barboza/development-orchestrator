...
5. **Detect project surface (silent)**: set `project_surface = visual | backend`. Force `visual` if the PRD has a substantive "Identidade Visual" section OR the Tech Spec has a substantive "Implementação Visual" section. Otherwise default to `backend`. Only when `project_surface = visual` does Step 3.5 append the visual validation task.
...
6. **Scope guideline**: each task should represent approximately 100–200 lines of production code change. Tasks estimated to exceed this should be split — this prevents context overflow during `do-execute-task`.
7. Wait for user approval before proceeding to Step 3.5.

**Step 3.5: Append Visual Validation Task (silent, deterministic)**
1. If `project_surface = backend`, SKIP this step entirely — proceed to Step 4.
2. If `project_surface = visual`:
   a. Compute `N` as the next integer after the highest task number approved in Step 3.
   b. Append a final task to the approved high-level list with the title: `N.0 Validação de Identidade Visual / CSS`.
   c. Notify the user that this task was added automatically because the feature declared visual identity (no approval needed — this is deterministic by rule). The user may remove it after generation by editing `tasks.md` and deleting the corresponding `[N]_task.md` file if they decide it isn't needed.
3. The content of this task comes from the dedicated template asset at `do-create-tasks/assets/visual-validation-task-template.md` (rendered in Step 4.7).
...
7. **Visual validation task (only when Step 3.5 appended one)**: read the dedicated template at `do-create-tasks/assets/visual-validation-task-template.md` and use it to write the `[N]_task.md` for the appended validation task. Replace every occurrence of the placeholder `<N>` with the actual integer assigned to this task. The template is content-complete — do NOT invent additional subtasks or critical tags.
8. Use format X.0 for main tasks, X.Y for subtasks **inside the file content only** — never in the filename.
9. Do NOT repeat implementation details already in the Tech Spec — reference it instead.
...
2. Present all generated files to the user. If a visual validation task was appended in Step 3.5, mention it explicitly so the user knows it was auto-added.
...
4. **COMPLIANCE CHECK**: Before responding to the user, verify:
    ...
    - If `project_surface = visual`: a visual validation task was appended (see Step 3.5). Otherwise N/A.
...
- [ ] Visual validation task appended (only if `project_surface = visual`; otherwise N/A).
...
## References
- Templates: `do-create-tasks/assets/tasks-template.md`, `do-create-tasks/assets/task-template.md`, `do-create-tasks/assets/visual-validation-task-template.md`
