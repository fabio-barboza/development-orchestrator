...
description: Itera sequencialmente sobre uma lista de tarefas de um PRD executando cada uma em SUBAGENTE ISOLADO via #runSubagent, garantindo contexto distinto entre tarefas. Não use para criação de PRD/TechSpec, QA, code review ou correção de bugs específicos.
tools:
  - read_file
  - list_dir
  - runSubagent
...
Cada task envolve leitura de PRD, TechSpec, código-fonte, edits, execução de testes e geração de artefatos de review. Se você executasse N tasks na sua própria sessão, o contexto acumularia leituras e tool results de todas elas e estouraria a janela. A delegação para subagente do GitHub Copilot cria uma sessão fresca por chamada (isolated context window), então cada task começa em "estado limpo".

Instruções textuais como "limpe o contexto" ou "releia do disco" **não liberam tokens** — somente a delegação real a um subagente faz isolamento.
...
#### 2.2. Delegação em subagente isolado

**Esta é a etapa que garante o isolamento de contexto.** Invoque o subagente `Agent Execute Task` (definido em `.github/agents/agent-execute-task.agent.md`) através do tool **`#runSubagent`** (ou pelo mecanismo de delegação automática do Copilot SDK), passando um prompt autocontido que inclua:
...
> Se o tool `#runSubagent` estiver indisponível na sua configuração do Copilot, **PARE** o loop e reporte ao usuário. Não tente executar a skill inline — isso reproduz o bug que esta arquitetura corrige.
...
#### 2.4. Próxima task
Cada delegação ao subagente já roda em contexto isolado, então não há "limpeza" a fazer no orquestrador além de emitir uma linha demarcadora e seguir:
