...
2. Identifique o diretório do PRD e das tasks individuais (ex.: `prds/prd-<nome>/tasks/`).
...
**Esta é a etapa que garante o isolamento de contexto.** Invoque a tool **Task** delegando ao agente `agent-execute-task` com um prompt autocontido que inclua:
...
**Aguarde o retorno do subagente** antes de prosseguir. Cada invocação da tool Task roda em **child session** isolada do opencode, sem herdar histórico nem tool results da task anterior — isto é o isolamento real.

> Se a tool Task estiver indisponível (permissão `task: allow` ausente, configuração quebrada), **PARE** o loop e reporte ao usuário. Não tente executar a skill inline — isso reproduz o bug que esta arquitetura corrige.
...
2. **Um subagente novo por task.** Sempre via Task delegando ao agente `agent-execute-task`. Nunca reutilize um subagente para múltiplas tasks. Nunca execute a skill inline.
...
[Task tool invocado delegando ao agent agent-execute-task para 1.0]
=== TASK 1.0 CONCLUÍDA ===
--- próxima task ---
=== INICIANDO TASK 2.0 (2/4) ===
[Task tool invocado delegando ao agent agent-execute-task para 2.0]
...
```
